...
--@Descripción: Trigger DML para SERVICIO_LAPTOP (Nodo Este - ensbdd_s2)
...
begin
    case
        when inserting then
            -- 1. LOCAL: Verifica si la laptop está en el Este (LAPTOP_F4)
            select count(*) into v_count from laptop_f4 where laptop_id = :new.laptop_id;
            if v_count > 0 then
                insert into servicio_laptop_f2(num_servicio, sucursal_id, laptop_id, diagnostico, importe, factura)
