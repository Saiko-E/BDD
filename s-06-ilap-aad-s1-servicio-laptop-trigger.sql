...
--@Descripción: Trigger DML para SERVICIO_LAPTOP (Nodo Oeste - aadbdd_s1)
...
begin
    case
        when inserting then
            -- 1. LOCAL: Verifica si la laptop está en el Oeste (LAPTOP_F3)
            select count(*) into v_count from laptop_f3 where laptop_id = :new.laptop_id;
            if v_count > 0 then
                insert into servicio_laptop_f3(num_servicio, sucursal_id, laptop_id, diagnostico, importe, factura)
                values (:new.num_servicio, :new.sucursal_id, :new.laptop_id, :new.diagnostico, :new.importe, :new.factura);
            else
                -- 2. REMOTO: Verifica si la laptop está en el Norte (LAPTOP_F1)
                ...
                if v_count > 0 then
                    ...
                    -- 3. REMOTO: Verifica si la laptop está en el Este (LAPTOP_F4)
