...
--@Descripción: Trigger DML para SUCURSAL_TALLER (Nodo Sur - aadbdd_s2)
...
begin
    case
        when inserting then
            -- 1. LOCAL: Busca en el Sur (F4)
            select count(*) into v_count from sucursal_f4 where sucursal_id = :new.sucursal_id;
            if v_count > 0 then
                insert into sucursal_taller_f4(sucursal_id, tel_atencion, dia_descanso) 
                values (:new.sucursal_id, :new.tel_atencion, :new.dia_descanso);
            else
                -- 2. REMOTO: Busca en el Norte (F1)
                select count(*) into v_count from sucursal_f1 where sucursal_id = :new.sucursal_id;
                if v_count > 0 then
                    insert into sucursal_taller_f1(sucursal_id, tel_atencion, dia_descanso) 
                    values (:new.sucursal_id, :new.tel_atencion, :new.dia_descanso);
                else
                    -- 3. REMOTO: Busca en el Este (F2)
                    select count(*) into v_count from sucursal_f2 where sucursal_id = :new.sucursal_id;
                    if v_count > 0 then
                        insert into sucursal_taller_f2(sucursal_id, tel_atencion, dia_descanso) 
                        values (:new.sucursal_id, :new.tel_atencion, :new.dia_descanso);
                    else
                        -- 4. REMOTO: Busca en el Oeste (F3)
                        select count(*) into v_count from sucursal_f3 where sucursal_id = :new.sucursal_id;
                        if v_count > 0 then
                            insert into sucursal_taller_f3(sucursal_id, tel_atencion, dia_descanso) 
                            values (:new.sucursal_id, :new.tel_atencion, :new.dia_descanso);
                        else
